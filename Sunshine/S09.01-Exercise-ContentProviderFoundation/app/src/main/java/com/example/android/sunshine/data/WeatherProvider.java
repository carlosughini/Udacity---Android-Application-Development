...
//  COMPLETED (5) Create static constant integer values named CODE_WEATHER & CODE_WEATHER_WITH_DATE to identify the URIs this ContentProvider can handle
    ...
    /** URI matcher code for the content URI for a single weather date in the table */
...
//  COMPLETED (6) Write a method called buildUriMatcher where you match URI's to their numeric ID
     ...
     * then use .addURI to add matches.
     */
    public static UriMatcher buildUriMatcher() {

        // Initialize a empty UriMatcher with no matches by passing in NO_MATCH to the constructor
        final UriMatcher uriMatcher = new UriMatcher(UriMatcher.NO_MATCH);

        /*
          The two calls below add match for the weather directory and a single weather date
         */
...
//      COMPLETED (9) Handle queries on both the weather and weather with date URI
        ...
        // Initialize cursor that will hold the result of the query
        Cursor cursor;

        // Figure out which URI matcher can match the URI to a specific code
        int match = sUriMatcher.match(uri);
        switch (match) {
            case CODE_WEATHER:
                cursor = sqLiteDatabase.query(WeatherEntry.TABLE_NAME,
                        projection,
                        selection,
                        selectionArgs,
                        null,
